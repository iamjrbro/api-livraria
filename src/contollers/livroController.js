...
class LivroController {
  ...
  //await sem cosnt poq não terá retorno, só executará a função
  static async cadastrarLivro(req, res) {
    try {
      const novoLivro = await livroModel.create(req.body);
      res
        .status(201)
        .json({ message: "Cadastrado com sucesso!", livro: novoLivro });
    } catch (error) {
      res
        .status(500)
        .json({ message: `${error.message} - FALHA NO CADASTRO DO LIVRO` });
    }
  }
